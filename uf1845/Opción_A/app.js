...
async function main() {
  ...
  try {
    ...
    console.log("******************** Consulta de ejemplo: Encuentra todas las películas cuyo idioma sea 'Ukrainian', ordenadas de forma decreciente por año de publicación ********************");

    const ukrainianMovies = await collection
      .find({ languages: "Ukrainian" }, { projection: { title: 1, year: 1, languages: 1, _id: 0 } })
    ...
    console.log('******************** Consulta 1: Encuentra 3 películas del género "Action", mostrando solo título y género.********************');
    const actionMovies = await collection
      .find({ genres: "Action" }, { projection: { title: 1, genres: 1, _id: 0 } })
      .sort({ year: -1 })
      .limit(3)
      .toArray();
    ...
    console.log("******************** Consulta 2: Encuentra 3 películas dirigidas por 'Christopher Nolan', mostrando solo título y director ********************");

    const nolanMovies = await collection
      .find({ directors: "Christopher Nolan" }, { projection: { title: 1, directors: 1, _id: 0 } })
      .sort({ year: -1 })
      .limit(3)
      .toArray();
    ...
    console.log("******************** Consulta 3: Encuentra 3 películas de los años 2000 o más recientes, mostrando título y año ********************");
    const recentMovies = await collection
      .find(
        {
          year: { $gte: 2000 },
        },
    ...
    console.log("******************** Consulta 4: Encuentra 3 películas ordenadas por fecha de lanzamiento en ordenascendente. Las más antiguas de toda la base de datos ********************");
    const orderedMovies = await collection
      .find({ year: { $exists: true, $ne: null } }, { projection: { title: 1, year: 1, _id: 0 } })
      .sort({ year: 1 })
      .limit(3)
      .toArray();
    ...
    console.log("******************** Encuentra 3 películas con una calificación mayor a 8.5 y más de 10000 votos en IMDb, mostrando solo título, año y calificación. Ordénalas de forma descendiente por calificación ********************");

    const topRatedMovies = await collection
      .find(
        {
          "imdb.rating": { $gte: 8.5 },
          "imdb.votes": { $gt: 10000 },
